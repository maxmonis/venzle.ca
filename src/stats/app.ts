...
export function mountStatsPage(main: HTMLElement) {
  let animationFrameIds: Array<number> = [];
  ...
  subheading.textContent = "Daily Puzzles on this Device";

  main.replaceChildren(heading, subheading);
  ...
  results = results.concat(
    games.flatMap(({ guesses, hintsUsed, index, status }) => {
      if (
        status == "pending" ||
        results.some((result) => result.index == index)
      ) {
        return [];
      }
  ...
  graphContainer.append(guessDistributionGraph, hintDistributionGraph);
  ...
  guessDistributionGraph
    ...
    .forEach((bar, i) => {
      ...
      timeoutIds.push(
        window.setTimeout(
          () => {
            barFill.style.width = `calc(${(count / maxCount) * 100}% - ${
              (count / maxCount) * 1.25
            }rem)`;
  ...
  return () => {
    for (let animationFrameId of animationFrameIds) {
  ...
  function animateNumber(
    ...
    function step(timestamp: number) {
      ...
      element.textContent =
        // partial update until the animation reaches its final value
        (decimalPlaces
