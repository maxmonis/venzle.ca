...
export function mountSharePage(main: HTMLElement, navigate: Navigate) {
  let active = true;
  ...
  let storageGame = games.find(
    (game) => game.index == index && game.status == "solved",
  );
...
${game.guesses
  ...
  downloadForm.addEventListener("submit", (e) => {
    ...
    anchor.href = certificateCanvas.toDataURL(`image/${format}`);
    anchor.download = `certificate.${format}`;
    anchor.click();
    anchor.remove();
   ...
   */
  async function appendCertificate(currentGame: Game) {
    ...
    let hintsCount = Object.values(currentGame.hintsUsed).filter(
      Boolean,
    ).length;
    let guessesUsed = currentGame.guesses.length;
    ...
    cursorY = wrapText(ctx, currentGame.title, centerX, cursorY, 800, 80);
    cursorY += 96;
    ...
    ctx.fillText(currentGame.creator, width * 0.325, footerY - 36);
    ctx.fillText(
      new Intl.DateTimeFormat(undefined, {
        dateStyle: "long",
      }).format(
        currentGame.timestamp ? new Date(currentGame.timestamp) : new Date(),
      ),
      width * 0.675,
      footerY - 36,
    );
